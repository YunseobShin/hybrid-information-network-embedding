...
def make_dictionaries(pages, exclude):
    wiki_dic = {}
    i_t = {}
    t_i = {}
...
class WikiParser():
    ...
    def __init__(self, input_file):
        with open(input_file, 'r') as f:
            self.wiki = f.read()
        self.pages = self.wiki.split('</page>')
        ...
        self.wiki_dic, self.t_i, self.i_t = make_dictionaries(self.pages, self.exclude)
    ...
    def catch_link(self, output_file):
        ...
        for title in tqdm(self.wiki_dic):
            if title in self.t_i:
                s=self.t_i[title]
            else:
                break
            page = self.wiki_dic[title]
    ...
    def catch_contents(self, text_file, output_json, sample_nodes):
        sample_nodes = [int(x) for x in sample_nodes]
        docs = open(text_file, 'r').read().split('</doc>')
        title_contents = {}
...
class WikiSampler():
    def __init__(self, i_t, t_i, edge_file, sample_size, sample_edges_file):
        self.t_i = t_i
        self.i_t = i_t
        self.edge_file = edge_file
        self.sample_size = sample_size
        self.sample = sample_by_pagerank(self.edge_file, self.sample_size)
    ...
    def find_links_from_sample(self, pool_size=1):
        ...
        tic()
        pool.map(self.write_sample_parallel, edges)
        toc()
