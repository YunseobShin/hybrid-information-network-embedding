...
class Node2VecEmbedder():
    def __init__(self, input_file, output_file, dimensions=256, walk_length=80, num_walks=20, workers=1, p=0.25, q=1, window_size=15, min_count=1, batch_words=4):
        ...
        if self.workers > 1:
            print('Warning: parallel computing for over 40K nodes may causes memory issue')
        self.p = p
        self.q = q
    ...
    def train(self):
        ...
        model.wv.save_word2vec_format(self.output_file)
...
class Hybrid():
    def __init__(self, output_file, alpha, t_i, i_t, nv_file, dv_file):
        ...
        self.common_idx = make_common_index(self.t_i, self.i_t, self.nv, self.dv)

    def mix_nv_dv(self):
        new_embedding = KV(vector_size = self.nv.vector_size)
        for key in tqdm(self.common_idx):
            if key not in self.dv.doctags.keys():
                continue
            new_embedding[key] = self.alpha * self.dv[key] + (1-self.alpha) * self.nv[self.t_i[key]]
